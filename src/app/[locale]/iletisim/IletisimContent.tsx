"use client";

import { Mail, Phone, MapPin, Clock, MessageCircle } from "lucide-react";
import { Breadcrumb } from "@/components/layout/Breadcrumb";
import { PageHero } from "@/components/layout/PageHero";
import { ContactForm } from "./ContactForm";
import { useT } from "@/lib/i18n/I18nProvider";

export function IletisimContent() {
  const t = useT();

  const CONTACT_ITEMS = [
    {
      icon: Mail,
      label: t.page.iletisim.contact_label_eposta,
      value: "info@tripandtick.com",
      href: "mailto:info@tripandtick.com",
      note: t.page.iletisim.contact_note_eposta,
    },
    {
      icon: Phone,
      label: t.page.iletisim.contact_label_telefon,
      value: "+90 537 464 78 61",
      href: "tel:+905374647861",
      note: t.page.iletisim.contact_note_telefon,
    },
    {
      icon: MessageCircle,
      label: t.page.iletisim.contact_label_whatsapp,
      value: "+90 537 464 78 61",
      href: "https://wa.me/905374647861",
      note: t.page.iletisim.contact_note_whatsapp,
    },
    {
      icon: MapPin,
      label: t.page.iletisim.contact_label_ofis,
      value: "Göreme Merkez, Nevşehir 50180",
      href: "https://maps.google.com/?q=Goreme+Nevsehir",
      note: t.page.iletisim.contact_note_ofis,
    },
  ];

  return (
    <>
      <PageHero
        tag={t.page.iletisim.pagehero_tag_iletisim}
        title={t.page.iletisim.pagehero_title_bizimle}
        highlight={t.page.iletisim.pagehero_highlight_iletisime_gec}
        description={t.page.iletisim.pagehero_description_sorulariniz_ozel}
      />

      <Breadcrumb items={[{ name: t.page.iletisim.breadcrumb_iletisim, href: "/iletisim" }]} />

      <section className="section-padding bg-slate-50">
        <div className="container-main">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
            <div>
              <h2 className="text-2xl font-extrabold text-slate-900 mb-6">
                {t.page.iletisim.iletisim_bilgileri}
              </h2>
              <div className="space-y-4 mb-8">
                {CONTACT_ITEMS.map((c) => (
                  <a
                    key={c.label}
                    href={c.href}
                    target={c.href.startsWith("http") ? "_blank" : undefined}
                    rel={c.href.startsWith("http") ? "noopener noreferrer" : undefined}
                    className="flex gap-4 p-4 bg-white rounded-xl border border-slate-200 hover:border-accent hover:shadow-card transition-all"
                  >
                    <div className="w-12 h-12 bg-primary/[0.08] rounded-lg flex items-center justify-center flex-shrink-0">
                      <c.icon className="w-5 h-5 text-primary" />
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-wide text-slate-500 font-bold">
                        {c.label}
                      </div>
                      <div className="font-bold text-slate-900">{c.value}</div>
                      <div className="text-xs text-slate-500 mt-0.5">{c.note}</div>
                    </div>
                  </a>
                ))}
              </div>

              <div className="bg-white rounded-2xl border border-slate-200 p-4 mb-6">
                <div className="flex items-center gap-2 mb-3 text-sm font-bold text-slate-700">
                  <Clock className="w-4 h-4 text-accent" />
                  <span>{t.page.iletisim.calisma_saatleri}</span>
                </div>
                <ul className="text-sm text-slate-600 space-y-1">
                  <li>{t.page.iletisim.whatsapp_posta_24}</li>
                  <li>{t.page.iletisim.telefon_09_00_22_00}</li>
                  <li>{t.page.iletisim.acil_iptal_degisiklik_24}</li>
                </ul>
              </div>

              <div className="rounded-2xl overflow-hidden border border-slate-200 shadow-card">
                <iframe
                  title={t.page.iletisim.iframe_title_trip_tick}
                  src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3175.5!2d34.8289!3d38.6431!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x0%3A0x0!2sG%C3%B6reme!5e0!3m2!1str!2str!4v1736000000000"
                  width="100%"
                  height="360"
                  style={{ border: 0 }}
                  loading="lazy"
                  referrerPolicy="no-referrer-when-downgrade"
                  allowFullScreen
                />
              </div>
              <a
                href="https://maps.google.com/?q=G%C3%B6reme,%20Nev%C5%9Fehir,%20T%C3%BCrkiye"
                target="_blank"
                rel="noopener noreferrer"
                className="mt-3 inline-flex items-center gap-2 px-4 py-2.5 bg-primary text-white rounded-xl text-sm font-bold hover:bg-primary-dark transition-colors"
              >
                <MapPin className="w-4 h-4" /> {t.page.iletisim.yol_tarifi}
              </a>
            </div>

            <div>
              <div className="bg-white rounded-2xl border border-slate-200 p-6 lg:p-8">
                <h2 className="text-2xl font-extrabold text-slate-900 mb-2">
                  {t.page.iletisim.mesaj_gonder}
                </h2>
                <p className="text-sm text-slate-600 mb-6">
                  {t.page.iletisim.form_doldurun_en_gec_saat}
                </p>
                <ContactForm />
              </div>
            </div>
          </div>
        </div>
      </section>
    </>
  );
}
